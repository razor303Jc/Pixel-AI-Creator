...
test.describe('Template Delete Fix Tests', () => {
  ...
  test('should allow deletion of duplicated templates', async ({ page }) => {
    ...
    const systemTemplate = page.locator('[data-testid^="duplicate-system-template-"]').first();
    if (await systemTemplate.count() > 0) {
      ...
      const duplicatedCard = page.locator('[data-testid^="template-card-"]').filter({ hasText: '(Copy)' }).first();
      ...
      const dropdownToggle = page.locator(`[data-testid="template-dropdown-${templateId}"]`);
      await expect(dropdownToggle).toBeVisible();
      await dropdownToggle.click();
      
      // Check that delete option is available
      const deleteOption = page.locator(`[data-testid="delete-template-${templateId}"]`);
      ...
      await page.waitForSelector('[data-testid="delete-modal"]', { timeout: 5000 });
  ...
  test('should show proper dropdown options for user templates', async ({ page }) => {
    // Wait for templates to load
    await page.waitForSelector('[data-testid^="template-card-"]', { timeout: 10000 });
    
    // Find a template with dropdown (user template)
    const userTemplateDropdown = page.locator('[data-testid^="template-dropdown-"]').first();
  ...
  test('should not show dropdown for system templates', async ({ page }) => {
    ...
    const systemTemplateCard = page.locator('[data-testid^="template-card-"]').filter({ hasText: 'System' }).first();
    
    if (await systemTemplateCard.count() > 0) {
      // System templates should not have dropdown, only duplicate button
      const dropdownInCard = systemTemplateCard.locator('[data-testid^="template-dropdown-"]');
      await expect(dropdownInCard).not.toBeVisible();
      
      // But should have duplicate button
      const duplicateButton = systemTemplateCard.locator('[data-testid^="duplicate-system-template-"]');
