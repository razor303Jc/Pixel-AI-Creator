...
test.describe("RazorFlow Human-like Workflow Tests", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="sidebar-templates"]', {
      timeout: 10000,
    });
  ...
  test("should complete human-like workflow: create client then template", async ({
    ...
    await page.waitForSelector('[data-testid="add-client-btn"]', {
      timeout: 5000,
    });
    ...
    await page.fill(
      ...
      "RazorFlow is an AI automation platform."
    );

    await page.click('[data-testid="add-qa-btn"]');
    await page.fill(
      '[data-testid="qa-question-1"]',
      "How do we handle support tickets?"
    );
    await page.fill(
      '[data-testid="qa-answer-1"]',
      "We prioritize tickets based on client tier and urgency."
    );
    ...
    await page.fill(
      '[data-testid="tool-calendar-apikey"]',
      "calendar-api-key-test"
    );

    await page.check('[data-testid="tool-web-search-toggle"]');
    await page.fill(
      '[data-testid="tool-web-search-apikey"]',
      "search-api-key-test"
    );
  ...
  test("should validate form fields with proper error handling", async ({
    ...
    await expect(
      page.locator('[data-testid="instructions-error"]')
    ).toBeVisible();
...
test.describe("Advanced Template Features", () => {
  test.beforeEach(async ({ page }) => {
    // Navigate to the application
    await page.goto("http://localhost:3002");
  ...
  test("should open create template modal and display new sections", async ({
    page,
  }) => {
  ...
  test("should manage Q&A training pairs", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="training-question-0"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="training-answer-0"]')
    ).toBeVisible();
    ...
    await expect(
      page.locator('[data-testid="training-question-1"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="training-answer-1"]')
    ).toBeVisible();
    ...
    await page.fill(
      ...
      "I can assist with information, analysis, and task completion."
    );

    // Add third Q&A pair
    await page.click("text=Add Q&A Pair");
    await expect(
      page.locator('[data-testid="training-question-2"]')
    ).toBeVisible();
  ...
  test("should toggle and configure tools", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="tool-web-search-api-key"]')
    ).toBeVisible();
    await page.fill(
  ...
  test("should create complete template with advanced features", async ({
    page,
  }) => {
    await page.click('[data-testid="add-template-btn"]');
    await page.waitForSelector('[data-testid="create-template-modal"]');
    ...
    await page.fill(
      '[data-testid="template-description-input"]',
      "A comprehensive AI assistant with multiple tools and training data for enhanced capabilities."
    );
    ...
    await page.fill(
      '[data-testid="template-instructions-input"]',
      "You are an advanced AI assistant with access to multiple tools and specialized training. Use your tools appropriately and refer to your training data for specific responses."
    );

    // Add tags
    await page.fill(
      '[data-testid="template-tags-input"]',
      "advanced, ai, tools, training, technical"
    );
    ...
    await page.fill(
      '[data-testid="training-answer-0"]',
      "I have access to web search, email, calendar, database, and various other tools to assist you."
    );
    ...
    await page.fill(
      '[data-testid="training-answer-1"]',
      "I analyze technical questions carefully and use appropriate tools and my specialized training to provide accurate, detailed responses."
    );
    ...
    await expect(
      page.locator(
        'text=Template "Advanced AI Assistant" created successfully!'
      )
    ).toBeVisible();
  ...
  test("should validate Q&A pairs properly", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Please fix the errors below before submitting")
    ).toBeVisible();

    // Complete the Q&A pair
    await page.fill(
      '[data-testid="training-answer-0"]',
      "Test answer for the question"
    );

    // Now submission should work
    await page.click('[data-testid="submit-template-btn"]');
    await expect(
      page.locator('text=Template "Test Template" created successfully!')
    ).toBeVisible();
  });
    ...
    for (const tool of tools) {
      ...
      if (tool !== "file-system") {
        if (tool === "database") {
          ...
          await expect(
            page.locator(`[data-testid="tool-${tool}-host"]`)
          ).toBeVisible();
        } else {
          // Most tools have API key field
          const apiKeyField = page.locator(
            `[data-testid="tool-${tool}-api-key"], [data-testid="tool-${tool}-key"]`
          );
