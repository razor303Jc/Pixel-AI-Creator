...
test.describe("Debug Templates Page", () => {
  test("take screenshot and debug page content", async ({ page }) => {
    ...
    const loginVisible = await page.locator('button:has-text("Login"), a:has-text("Login"), .btn:has-text("Login")').isVisible();
    const signUpVisible = await page.locator('button:has-text("Sign Up"), a:has-text("Sign Up"), .btn:has-text("Sign Up")').isVisible();
    ...
    const navElements = await page.locator('nav, .navbar, .nav-tabs, .tab-pane').count();
    ...
    const cardElements = await page.locator('.card, .template, .item, .content').count();
