    ...
    try:
        ...
        async with httpx.AsyncClient(app=app, base_url="http://test") as client:
            ...
            if response.status_code == 200:
                ...
                client_data = {
                    "name": "Test Client",
                    "email": f"testclient_{int(datetime.now().timestamp())}@example.com",
                    "company": "Test Company",
                }

                create_response = await client.post(
                    "/api/clients", json=client_data, headers=headers
                )
...
async def main():
    ...
    if db_init_success:
        ...
        if auth_success and auth_token:
            test_results.append(
                ("Protected Endpoints", await test_protected_endpoints(auth_token))
            )
    else:
