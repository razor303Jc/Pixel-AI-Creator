...
class PermissionBase(BaseModel):
    ...
    permission_type: str = Field(
        ..., description="Permission type (read, write, delete, etc.)"
    )
    resource_type: str = Field(
        ..., description="Resource type this permission applies to"
    )
    scope: str = Field(
        ..., description="Permission scope (global, organization, team, resource)"
    )


class PermissionCreate(PermissionBase):
    is_system_permission: bool = Field(
        False, description="Whether this is a system permission"
    )
...
class RoleCreate(RoleBase):
    ...
    max_assignees: Optional[int] = Field(
        None, description="Maximum number of users that can have this role"
    )
    is_assignable: bool = Field(
        True, description="Whether this role can be assigned to users"
    )
...
async def revoke_user_role(
    ...
    try:
        success = await authorization_service.revoke_role(
            user_id=user_id, role_id=role_id, revoked_by=current_user.id, db=db
        )
...
async def get_user_permissions(
    ...
    try:
        permissions = await authorization_service.get_user_permissions(
            user_id=user_id, resource_type=resource_type, resource_id=resource_id, db=db
        )

        return UserPermissionsResponse(user_id=user_id, **permissions)
...
@router.post(
    "/users/{user_id}/check-permission", response_model=PermissionCheckResponse
)
    ...
    try:
        ...
        if user_id != current_user.id:
            has_check_permission = await authorization_service.check_permission(
                user=current_user, permission_name="user.read", db=db
            )
...
async def get_audit_logs(
    ...
    try:
        ...
        query = (
            query.order_by(desc(PermissionAudit.performed_at)).offset(skip).limit(limit)
        )
