...
async def create_chatbot(
    ...
    try:
        ...
        logger.info(f"Chatbot project created: {new_project.name} by user {current_user['email']}")
        return new_project
...
async def list_chatbots(
    ...
    try:
        ...
        logger.info(f"Listed {len(projects)} chatbot projects for user {current_user['email']}")
        return projects
...
async def get_chatbot(
    ...
    try:
        ...
        logger.info(f"Retrieved chatbot project {project_id} by user {current_user['email']}")
        return project
...
async def update_chatbot(
    ...
    try:
        ...
        logger.info(f"Updated chatbot project {project_id} by user {current_user['email']}")
        return project
...
async def update_chatbot_status(
    ...
    try:
        ...
        logger.info(f"Updated chatbot project {project_id} status to {new_status} by user {current_user['email']}")
...
async def delete_chatbot(
    ...
    try:
        ...
        logger.info(f"Deleted chatbot project {project_id} by user {current_user['email']}")
...
async def get_chatbot_stats(
    ...
    try:
        ...
        stats = {
            ...
            "days_in_development": (datetime.utcnow() - project.created_at).days if project.created_at else 0,
        ...
        logger.info(f"Retrieved stats for chatbot project {project_id} by user {current_user['email']}")
        return stats
...
async def get_chatbot_dashboard(
    ...
    try:
        ...
        type_result = await db.execute(
            select(Project.assistant_type, func.count(Project.id))
            .group_by(Project.assistant_type)
        )
        type_breakdown = {assistant_type: count for assistant_type, count in type_result.all()}
        
        # Get projects by complexity
        complexity_result = await db.execute(
            select(Project.complexity, func.count(Project.id))
            .group_by(Project.complexity)
        )
        complexity_breakdown = {complexity: count for complexity, count in complexity_result.all()}
