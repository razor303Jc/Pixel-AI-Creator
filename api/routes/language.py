...
async def translate_conversation_message(
    ...
    if not request.message.strip():
        raise HTTPException(
            status_code=400,
            detail="Message is required for translation"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Conversation translation failed: {str(e)}"
        )
...
async def analyze_language_usage(
    ...
    if not messages:
        raise HTTPException(
            status_code=400,
            detail="At least one message is required for analysis"
        )
...
async def language_service_health() -> Dict[str, Any]:
    ...
    return {
        ...
        "capabilities": {
            "language_detection": True,
            "google_translate": language_service.translator is not None,
            "supported_languages_count": len(language_service.get_supported_languages()),
