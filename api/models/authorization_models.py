...
class Role(Base):
    ...
    scope = Column(String(50), default=PermissionScope.ORGANIZATION.value, nullable=False)
    max_assignees = Column(Integer, nullable=True)  # Limit number of users with this role
    ...
    __table_args__ = (
        ...
        CheckConstraint('max_assignees IS NULL OR max_assignees > 0', name='ck_role_max_assignees')
    )
...
class Permission(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint('permission_type', 'resource_type', 'scope', name='uq_permission_type_resource_scope')
    )
...
class UserPermission(Base):
    ...
    # Grant properties
    is_granted = Column(Boolean, default=True, nullable=False)  # Can be False for explicit deny
    ...
    __table_args__ = (
        ...
        UniqueConstraint('user_id', 'permission_id', 'resource_type', 'resource_id', 
...
def update_user_model():
    ...
    User.direct_permissions = relationship("UserPermission", foreign_keys=[UserPermission.user_id], 
                                         back_populates="user")
    User.organization_memberships = relationship("OrganizationMembership", foreign_keys=[OrganizationMembership.user_id])
    User.team_memberships = relationship("TeamMembership", foreign_keys=[TeamMembership.user_id])
