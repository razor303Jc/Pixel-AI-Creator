...
class DockerBuildManager:
    ...
    async def queue_build(
        self, project_id: int, user_id: int, chatbot_config: Dict[str, Any]
    ) -> str:
            ...
            try:
                ...
                if success:
                    ...
                    await self._update_project_status(
                        build_job.project_id, "completed", 100
                    )
                    ...
                    logger.error(
                        f"Build {build_job.id} failed: {build_job.error_message}"
                    )
    ...
    async def _execute_build(self, build_job: BuildJob) -> bool:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                # Build Docker image
                image_tag = (
                    f"pixel-ai-chatbot-{build_job.project_id}:{build_job.id[:8]}"
                )
                image = await self._build_docker_image(
                    build_job, build_context, image_tag
                )
...
'''
    ...
    async def _build_docker_image(
        self, build_job: BuildJob, build_context: Path, image_tag: str
    ):
        """Build Docker image"""
        try:
            ...
            image, build_logs = self.docker_client.images.build(
                path=str(build_context), tag=image_tag, rm=True, pull=True
            )
    ...
    async def _deploy_chatbot(self, build_job: BuildJob, image_tag: str) -> bool:
        ...
        try:
            ...
            build_job.build_logs.append(
                f"Chatbot deployed successfully on port {api_port}"
            )
            return True
    ...
    async def _update_project_deployment(
        self, project_id: int, container_name: str, port: str, url: str
    ):
        ...
        try:
            ...
            async with get_db() as db:
                await db.execute(
                    ...
                    .where(Project.id == project_id)
                    .values(
                        deployment_info=deployment_info, updated_at=datetime.utcnow()
                    )
                )
                await db.commit()
        except Exception as e:
