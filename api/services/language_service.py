...
class LanguageService:
    ...
    def __init__(self):
        ...
        self.supported_languages = {lang.value: lang.name.replace('_', ' ').title() 
    ...
    def translate_text(
        ...
        self.logger.warning(f"No translation service available for {source_lang} -> {target_lang}")
        return TranslationResult(
            original_text=text,
            translated_text=text,
            source_language=source_lang,
    ...
    def get_language_statistics(self, messages: List[str]) -> Dict[str, Any]:
        ...
        for lang_data in language_counts.values():
            ...
            lang_data['avg_confidence'] = lang_data['total_confidence'] / lang_data['count']
        
        # Find primary language
        primary_language = max(
            language_counts.keys(),
            key=lambda x: language_counts[x]['count']
        ) if language_counts else 'en'
        ...
        return {
            ...
            'avg_detection_confidence': total_confidence / total_messages if total_messages > 0 else 0
        }
