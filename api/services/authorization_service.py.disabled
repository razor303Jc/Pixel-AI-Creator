...
class AuthorizationService:
    ...
    async def initialize_system_data(self, db: AsyncSession) -> None:
        ...
        try:
            ...
            for perm_name, (
                ...
                resource_type,
                scope,
            ) in self.system_permissions.items():
                ...
                if not existing.scalar_one_or_none():
                    permission = Permission(
                        name=perm_name,
                        display_name=perm_name.replace(".", " ")
                        .replace("_", " ")
                        .title(),
    ...
    async def check_permission(
        ...
        try:
            ...
            if await self._check_direct_permission(
                user.id, permission_name, resource_type, resource_id, db
            ):
                return True

            # Check role-based permissions
            if await self._check_role_permissions(
                user.id, permission_name, resource_type, resource_id, db
            ):
                return True

            # Check inherited permissions
            if await self._check_inherited_permissions(
                user.id, permission_name, resource_type, resource_id, db
            ):
                return True
            ...
            logger.error(
                f"Permission check failed for user {user.id}, permission {permission_name}: {e}"
            )
            return False
        ...
        try:
            ...
            result = await db.execute(
                ...
                .options(
                    ...
                    selectinload(User.direct_permissions).selectinload(
            ...
            for user_perm in user.direct_permissions:
                if user_perm.is_granted and (
                    not user_perm.expires_at or user_perm.expires_at > datetime.utcnow()
                ):
                    # Check resource match
                    if (
                        resource_type
                        and user_perm.resource_type
                        and user_perm.resource_type != resource_type
                    ):
                        continue
                    if (
                        resource_id
                        and user_perm.resource_id
                        and user_perm.resource_id != resource_id
                    ):
                        continue
            ...
            permissions["effective_permissions"] = list(
        ...
        try:
            ...
            if role.max_assignees:
                current_count = await db.execute(
                    select(func.count(user_roles.c.user_id)).where(
    ...
    async def revoke_role(
        self, user_id: int, role_id: int, revoked_by: int, db: AsyncSession = None
    ) -> bool:
    ...
    async def grant_permission(
        ...
        try:
            ...
            logger.error(
                f"Failed to grant permission {permission_id} to user {user_id}: {e}"
            )
            await db.rollback()
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def revoke_permission(
        ...
        try:
            ...
            logger.error(
                f"Failed to revoke permission {permission_id} from user {user_id}: {e}"
            )
            await db.rollback()
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def _is_super_admin(self, user: User, db: AsyncSession) -> bool:
        ...
        try:
            result = await db.execute(
                ...
                .join(user_roles)
                .where(
                    and_(user_roles.c.user_id == user.id, Role.name == "super_admin")
                )
            )
    ...
    def _clear_user_cache(self, user_id: int) -> None:
        ...
        cache_keys = [
            k for k in self._permission_cache.keys() if k.startswith(f"user_{user_id}_")
        ]
