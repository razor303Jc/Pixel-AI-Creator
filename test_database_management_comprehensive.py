...
class DatabaseManagementSystemTest:
    ...
    def test_basic_health(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.results["health_check"] = (False, f"HTTP {response.status_code}: {response.text}")
                return False
    ...
    def test_connection_stats(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                required_fields = ["total_connections", "active_connections", "idle_connections", "timestamp"]
                
                missing_fields = [field for field in required_fields if field not in data]
                if missing_fields:
                    self.results["connection_stats"] = (False, f"Missing fields: {missing_fields}")
                    return False
                
                self.results["connection_stats"] = (True, f"Stats: {data}")
                return True
            else:
                self.results["connection_stats"] = (False, f"HTTP {response.status_code}: {response.text}")
                return False
    ...
    def test_database_system_functionality(self):
        ...
        try:
            ...
            sessions_response = requests.get(f"{self.base_url}/api/sessions-test/", timeout=10)
            ...
            auth_working = auth_response.status_code in [200, 404]  # 404 is OK if endpoint doesn't exist
            
            functionality_score = sum([api_accessible, sessions_working, auth_working])
            
            if functionality_score >= 2:
                self.results["system_functionality"] = (True, f"Score: {functionality_score}/3 - API: {api_accessible}, Sessions: {sessions_working}, Auth: {auth_working}")
                return True
            else:
                self.results["system_functionality"] = (False, f"Score: {functionality_score}/3 - API: {api_accessible}, Sessions: {sessions_working}, Auth: {auth_working}")
                return False
    ...
    def test_database_configuration(self):
        ...
        try:
            ...
            health_response = requests.get(f"{self.base_url}/api/database/health", timeout=10)
            stats_response = requests.get(f"{self.base_url}/api/database/stats", timeout=10)
            ...
            if health_ok and stats_ok:
                ...
                if has_healthy_status and has_connection_info:
                    self.results["database_config"] = (True, f"Configuration OK - Health: {health_data}, Stats: {stats_data}")
                    return True
                else:
                    self.results["database_config"] = (False, f"Config issues - Health: {health_data}, Stats: {stats_data}")
                    return False
            else:
                self.results["database_config"] = (False, f"Endpoint errors - Health: {health_response.status_code}, Stats: {stats_response.status_code}")
                return False
